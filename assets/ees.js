...
var HashScroll = (function () {
  _createClass(HashScroll, null, [{
    key: 'normalizeHash',
    value: function normalizeHash(hash) {
      return hash.toString().replace(/([^a-z0-9]+)/gi, '');
    }
  }, {
    key: 'getSectionFromHash',
    value: function getSectionFromHash(hash) {
      var normalizedHash = HashScroll.normalizeHash(hash);
      var section = $('#' + normalizedHash);
      return section.length ? section : $('[name=' + normalizedHash + ']');
    }
  }]);

  function HashScroll(options) {
    _classCallCheck(this, HashScroll);

    this.options = deepmerge({ changeAnimationMs: 1000 }, options || {});
    this.window = $(window);
    this.lastHash = null;
    this.setUp();
  }
  ...
  _createClass(HashScroll, [{
    ...
    key: 'changeTo',
    value: function changeTo(newHash) {
      this.lastHash = newHash;
      var section = HashScroll.getSectionFromHash(this.lastHash);

      $('html, body').stop().animate({
        scrollTop: section.offset().top
      }, this.options.changeAnimationMs);
    }
  }, {
    key: 'canChangeTo',
    value: function canChangeTo(newHash) {
      return newHash !== this.lastHash && HashScroll.getSectionFromHash(newHash).length;
    }
  }, {
    key: 'onHashChange',
    value: function onHashChange() {
      var newHash = HashScroll.normalizeHash(window.location.hash);

      if (this.canChangeTo(newHash)) {
        this.changeTo(newHash);
      }
    }
  }]);
...
$(function () {
  (window.ees = window.ees || {}).hashScroll = new HashScroll();
});
(function($) {
  function Gmaps(options) {
    this.options = $.extend({}, Gmaps.defaultOptions, options || {});
  }

  Gmaps.defaultOptions = {
    transport: 'https',
  ...
  Gmaps.prototype = {
    load: function() {
      ...
      if (this.loadPromise) {
        ...
        var randomizedFunctionName = this.options.callback = 'onGoogleMapsReady' + Math.round(Math.random() * 1000);

        window[randomizedFunctionName] = function() {
          window[randomizedFunctionName] = this.loadPromise = undefined;
          deferred.resolve(window.google.maps);
        };
      ...
      return this.loadPromise = deferred.promise();
    },
    ...
    includeScript: function() {
      ...
      script.id = this.scriptId = 'ui_gmap_map_load_' + Math.floor((new Date().getTime()) / 1000);
      script.type = 'text/javascript';
      query = query.join('&');
      script.src = this.getScriptUrl(this.options) + query;

      return document.body.appendChild(script);
    },

    getScriptUrl: function() {
      if (this.options.china) {
        return 'http://maps.google.cn/maps/api/js?';
      } else {
        if (this.options.transport === 'auto') {
          return '//maps.googleapis.com/maps/api/js?';
        } else {
          return this.options.transport + '://maps.googleapis.com/maps/api/js?';
        }
      }
    }
  };
  ...
  $(function() {
    window.gmaps = window.gmaps || new Gmaps({
      key: $('#gmaps-key').val()
    });
  });
}(jQuery));
...
var Assignable = function Assignable(attributes) {
  _classCallCheck(this, Assignable);

  var self = this;

  Object.keys(attributes).forEach(function (key) {
    return self[key] = attributes[key];
  });
};

var StoreKind = (function (_Assignable) {
  _inherits(StoreKind, _Assignable);

  _createClass(StoreKind, null, [{
    key: 'add',
    value: function add(store) {
      this.stores[store.title] = store;
    }
  }, {
    key: 'getAll',
    value: function getAll() {
      ...
      return Object.keys(this.stores).map(function (id) {
        return _this.stores[id];
      });
    }
  }, {
    key: 'getOne',
    value: function getOne(id) {
      return this.stores[id.toLowerCase()];
    }
  }, {
    key: 'stores',
    get: function get() {
  ...
  function StoreKind() {
    _classCallCheck(this, StoreKind);
...
var Store = (function (_Assignable2) {
  _inherits(Store, _Assignable2);
  ...
  _createClass(Store, [{
    ...
    value: function toHTML() {
      ...
      if (this.phone) {
        node += '<p>Fone: ' + this.phone + '</p>';
      }

      return '<div class="store__info-window">' + node + '</div>';
    }
  }, {
    key: 'latLng',
    get: function get() {
    ...
    get: function get() {
      var address = this.address + ', ' + this.city;
...
var Map = (function () {
  function Map(gmaps, mapEl) {
    _classCallCheck(this, Map);

    this.gmaps = gmaps;
    this.mapEl = $(mapEl);
    this.stores = [];
    this._gmapDeferred = $.Deferred();
    this.gmapPromise = this._gmapDeferred.promise();

    this.centerMapToUserLocation();
    this._setUpStores();
    this.gmaps.load().then(this._onGmapsLoad.bind(this));
  }
  ...
  _createClass(Map, [{
    ...
    value: function centerMapToUserLocation() {
      ...
      this.gmapPromise.then(function (gmap) {
        _this2.userLocation().then(function (position) {
    ...
    key: 'addStoresToMap',
    value: function addStoresToMap(stores) {
      this.stores = stores;

      this.gmapPromise.then(function (gmap) {
        stores
        // Responsável por juntar as variáveis `store` e `map` dentro do objeto store
        .map(function (store) {
          return deepmerge({ store: store, map: gmap }, store.toMarker());
        })

        // Responsável por pegar o objeto `marker` retornado do map anterior e cria um marker do google maps
        .map(function (marker) {
          return new google.maps.Marker(marker);
        })

        // Responsável por criar o infoWindow do google maps e colocar a variável `marker`
        // acessível dentro do próprio infoWindow porque ele retorna infoWindow e não pode
        // perder o acesso ao `marker`
        .map(function (marker) {
          var infoWindow = new google.maps.InfoWindow(marker.store.toInfoWindow());
          infoWindow.marker = marker;
          return infoWindow;
        })

        // Responsável por adicionar o evento de click no marker p/ abrir o infoWindow
        .map(function (infoWindow) {
    ...
    value: function _setUpStores() {
      ...
      (_$ = $).when.apply(_$, _toConsumableArray(StoreKind.getAll().map(function (storeKind) {
        return storeKind.fetchStores();
      }))).then(function () {
        var _ref;

        for (var _len2 = arguments.length, storesPromise = Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
          storesPromise[_key2] = arguments[_key2];
        }

        return (_ref = []).concat.apply(_ref, _toConsumableArray(storesPromise.map(function (promise) {
          return promise[0];
        })));
...
$(function () {
  // Inicia os tipos `StoreKind`
  ['CityLar', 'Gazin'].forEach(function (title) {
    return new StoreKind({ title: title });
  });
...
$(document).ready(function () {
  $.getJSON("data/cidade.json", function (cidades) {
